...
def get_tensors(names, data_augm = True):
    ...
    for idx in range(0,len(names)):
        ...
        # Make hint
        # threshold = 235
        # gray = (np.clip(lineart,threshold,threshold+1)-threshold).astype(np.uint8)
        #_, gray_labels, _, _ = cv2.connectedComponentsWithStats(gray)
        #availible_region = np.where(lineart >254, True, False)

        
        

        #gray_t = gray[max_width:len(lineart)-max_width, max_width:len(lineart)-max_width]
        # availible_region = availible_region[max_width:len(lineart)-max_width, max_width:len(lineart)-max_width]
        #width = np.random.randint(2,max_width) 
        hint = np.zeros(color.shape).astype(np.float32)
        hint[:,:,0]-=256

        availible_region = np.array(np.where(lineart >254))
        ...
        #random_height = np.random.randint(1,max_width, size = sample_amount*2)
